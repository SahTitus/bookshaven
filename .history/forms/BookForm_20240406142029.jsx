...
import { articleValidationSchema, handleError } from '@utils';
import TextEditor from './Editor';
import { createArticle, updateArticle } from '@lib/actions/book.action';
import { categories } from '@lib/constants/categories';
import { useAppSelector } from '@redux/store';
import { Dropzone } from './DropZone';
import { useStateContex } from '@redux/StateProvider';
import { clearCachesByServerAction } from '@lib/clearCache';
...
const BookForm = ({ defaultFormValues, slug, revalidatePath }) => {
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      if (slug) {
        await updateArticle(values, user_id)
      } else {
        await createArticle(values, user_id)
      }
  ...
  return (
    <div className="bg-dark min-h-screen flex items-center justify-center">
      <Formik initialValues={defaultFormValues} validationSchema={articleValidationSchema} onSubmit={handleSubmit}>
        {({ values, setFieldValue }) => (
          <Form className="bg-zinc-900 shadow-md rounded px-2 md:px-8 pt-6 pb-8 mb-4 flex flex-col w-11/12 md:w-3/4 ">
            ...
            <button
              className=" bg-zinc-800 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline mt-4"
              onClick={() => clearCachesByServerAction(path)}
              type="button"
              aria-label='revalidate button'
              disabled={submissionStatus === 'loading'}
            >
              Revalidate Home
            </button>
