...
export const AuthForm = () => {
    ...
    const submit = async (values) => {
        handleAuthFormSubmit({ formData: values, isSignin: isSignIn, setError, setShowLoaderOverlay })
    };
        ...
        <div className={``}>
            <div className="relative bg-[#f7f7f7] flex-col mt-20 flex items-center justify-center select-none w-11/12  md:w-[70%] max-w-xl mx-auto text-center md:text-start rounded-lg border px-5 py-4  ">
                ...
                <div className='flex justify-between gap-4 mb-3 mt-5'>
                    ...
                    <p onClick={() => setIsSignIn((prev)=>!prev)} className={`${isSignIn ? 'border-b border-zinc-500 rounded-lg bg-oran text-white' : ''} flex items-center justify-center text-base font-bold  py-2 px-5 text-gray-800 cursor-pointer`}>{!isSignIn ?'Sign in':''}</p>
                    <p onClick={() => setIsSignIn(false)} className={`${isSignIn ? 'border-b border-zinc-500 rounded-lg bg-oran text-white' : ''} flex items-center justify-center text-base font-bold  py-2 px-5 text-gray-800 cursor-pointer`}>Sign up</p>
                </div>
            </div >
        </div>
    );
