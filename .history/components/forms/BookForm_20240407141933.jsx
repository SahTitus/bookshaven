...
const BookForm = ({ defaultFormValues, slug, revalidatePath }) => {
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      if (slug) {
        await updateBook(values, user?.id)
      } else {
        await createBook(values, user?.id)
      }
      ...
      <Formik initialValues={defaultFormValues} validationSchema={bookValidationSchema} onSubmit={handleSubmit}>
        {({ values, setFieldValue }) => (
          <Form className="bg-white rounded px-2 md:px-8 pt-6 pb-8 mb-4 flex flex-col w-11/12 md:w-3/4 ">
            ...
            <label className={labelStyles} htmlFor="genre">
              Genre
            </label>
            <Field
              as="select"
              ...
              onChange={(e) => {
                ...
                const genre = genres.find((c) => c.name === selectedGenre);
                setFieldValue('genre', selectedGenre);
                setFieldValue('genre_id', genre ? genre.genre_id : '');
            ...
            <Field className={fieldStyles} type="text" name="pages" />
