...
import { signInSchema, signUpSchema, adminSchema } from '@utils';
...
export const AuthForm = () => {
    ...
    const { setShowLoaderOverlay, isAdmin, setIsAdmin } = useStateContex();
    ...
    const validationSchema = isSignIn ? signInSchema : isAdmin ? adminSchema : signUpSchema

    const submit = async (values) => {
        console.log(values)
        handleAuthFormSubmit({ formData: values, isSignin: isSignIn, setIsAdmin, setError, router, setShowLoaderOverlay, userId:session?.data?.user?.id,isAdmin })
    };
        ...
        <div className={``}>
            <div className="relative bg-[#f7f7f7] flex-col mt-20 flex items-center justify-center select-none w-11/12  md:w-[70%] max-w-xl mx-auto text-center md:text-start rounded-lg border px-5 py-4  ">
                ...
                <p className={`text-black m-0 md:max-w-[6
                    0ch] text-lg md:text-base  mb-3`}>
                    Get the best experience on this app by signing in.
                </p>
                <Formik initialValues={initialValues.user} onSubmit={submit}>
                    {({ values, setFieldValue }) => (
                        <Form className="bg-[#f7f7f7]   shadow-md rounded px-4 py-5 gap-3 flex flex-col w-full">
                            {!isSignIn && <>
                                ...
                                <Field placeholder='Firstname' className={fieldStyles} type="text" name="firstName" />
                            ...
                            {!isAdmin && <><Field placeholder='Email' className={fieldStyles} type="text" name="email" />
                            ...
                            <button
                                className=" bg-oran text-white  font-bold py-3 rounded focus:outline-none focus:shadow-outline "
                                type="submit"
                ...
                <div className='flex items-center justify-between gap-4 mb-3 mt-5'>
                    {!isSignIn ? <p>Already have an Account?</p> : <p>New here?</p>}
                    <p onClick={() => setIsSignIn((prev) => !prev)} className={`text-oran flex items-center justify-center text-base font-bold   cursor-pointer`}>{!isSignIn ? 'Sign in' : 'Sign up'}</p>
                </div>
            </div >
        </div>
    );
