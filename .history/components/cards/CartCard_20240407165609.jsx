...
const CartCard = ({ book, cart_id, user, isLoading }) => {
  ...
  const handleCardSelect = (id) => {
    // Check if the item's ID exists in the array
    const idIndex = itemsToDeleteIds.indexOf(id);
    ...
    if (idIndex !== -1) {
        ...
        const updatedIds = [...itemsToDeleteIds.slice(0, idIndex), ...itemsToDeleteIds.slice(idIndex + 1)];
  ...
  console.log(itemsToDeleteIds.length)

  return (
    <div className={styles.cartCard__container}>
      <div className={styles.cartCard__wrapper}>
      <p
  onClick={() => handleCardSelect(book?._id)}
  className={styles.checkbox__wrapper}
>
  <input
    readOnly
    checked={itemsToDeleteIds.includes(book?._id)}
    id={`checkbox_${book?._id}`}
    type="checkbox"
  />
  <label htmlFor={`checkbox_${book?._id}`} className={styles.checkbox}></label>
</p>
