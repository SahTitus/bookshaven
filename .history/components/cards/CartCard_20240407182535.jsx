...
const CartCard = ({ book, cart_id, user, isLoading,sele, setSelecetdIds }) => {
  ...
  const handleCardSelect = (id) => {
    const idExist = sele.includes(id);

    if (idExist) {
      const updatedIds = sele?.filter((item) => item !== id);
    ...
    <div className={styles.cartCard__container}>
      <div className={styles.cartCard__wrapper}>
        <p
          ...
          <input
            readOnly
            checked={sele.includes(book?._id)}
            id={styles.checkbox}
            type="radio" />
        </p>
