...
const CartCard = ({ book, cart_id, user, isLoading,itemsToDeleteIds, setSelecetdIds }) => {
  ...
  const handleCardSelect = (id) => {
    const idExist = itemsToDeleteIds.includes(id);

    if (idExist) {
      const updatedIds = itemsToDeleteIds?.filter((item) => item !== id);
      setSelecetdIds(updatedIds);
    } else {
      setSelecetdIds(prevIds => [...prevIds, id]);
    }
  };
