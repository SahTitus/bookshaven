...
const CartCard = ({ book, cart_id, user, isLoading,selectedI, setSelecetdIds }) => {
  ...
  const handleCardSelect = (id) => {
    const idExist = selectedI.includes(id);

    if (idExist) {
      const updatedIds = selectedI?.filter((item) => item !== id);
    ...
    <div className={styles.cartCard__container}>
      <div className={styles.cartCard__wrapper}>
        <p
          ...
          <input
            readOnly
            checked={selectedI.includes(book?._id)}
            id={styles.checkbox}
            type="radio" />
        </p>
