  ...
  return (
    ...
    >
      ...
      <div className={styles.cart__footer}>
        <p>
          {" "}
          <input readOnly id={styles.checked} type="radio" /> All
        </p>
        <div className={styles.footer__right}>
          <strong>GHS{userCart?.total_price} </strong>
          <button className={styles.footer__button}>Check Out(9)</button>
        </div>
      </div>
      <div className={styles.footer__right}>
        <strong>GHS{total?.totalPrice?.toFixed(2)} </strong>
        <button className={styles.footer__button}>Check Out(total?.totalQuantity)</button>
      </div>
    </div>
  );
};
