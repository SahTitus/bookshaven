...
export const MenuItems = ({ toggleSidebar, isNav, isLarge, openCartSpecsModal, totalCartQnty }) => {
    return (
        <div className={` flex ${isLarge ? 'flex-row justify-end items-cet gap-4 translate-x-0' : 'flex-col'}  transition-transform transform  ${isNav ? 'translate-x-0 ' : '-translate-x-full'} `}>
            {navRoutes.map(item =>
                <Link
