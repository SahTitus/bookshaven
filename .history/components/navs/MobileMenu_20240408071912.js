...
import { categories, genres } from '@lib/constants/categories';
import Link from 'next/link';

const MobileMenu = ({ isOpen, toggleSidebar, logOut, signIn, user, setIsAdmin }) => {
            ...
            <div className="absolute right-0  overflow-hidden  w-11/12 h-full ">
                ...
                <div className="relative flex flex-col items-start justify-start w-full h-full  px-4 pt-2 rounded-s-lg border border-neutral-600 bg-slate-100 bg-opacity-100 z-[1000000]">
                    ...
                    <div className='flex justify-between gap-4 mb-3 mt-5'>
                        <p onClick={() => setIsGenre(true)} className={`${isGenre ? 'border-b border-zinc-500 rounded-lg bg-oran text-white' : ''} flex items-center justify-center text-base font-bold  py-2 px-5 text-gray-800 cursor-pointer`}>Genre</p>
                        <p onClick={() => setIsGenre(false)} className={`${!isGenre ? 'border-b border-zinc-700 rounded-lg bg-oran text-white' : ''} flex items-center justify-center text-base font-bold  py-2 px-5 text-gray-800 cursor-pointer`}>Category</p>
                    </div>
                    <InputWithDropdown toggleSidebar={toggleSidebar} isSidebar={true} label={isGenre ? 'Genre' : 'Category'} options={isGenre ? genres : categories} />
                    ...
                    <div className='flex items-center justify-between w-full mt-4 py-3 px-4 shadow shadow-slate-700 rounded'>
                        {user?.id ?
                            ...
                            (     <Link
                                onClick={() => {setIsAdmin(false), toggleSidebar()}}
                                href='/auth'>
                                <button aria-label='Sign in' className=' md:flex bg-gradient-to-br from-oran to-[#fb0] text-white px-3 py-3 rounded-md text-lg'>Sign in</button>
                            </Link>)}
                        <Link
                            onClick={() => {setIsAdmin(true), toggleSidebar()}}
                            href='/auth'>
                            <button aria-label='Sign up' className=' md:flex bg-gradient-to-br from-oran to-[#fb0] text-white px-3 py-3 rounded-md text-lg'>Admin</button>
                        </Link>
                    </div>
                </div>
            </div>
