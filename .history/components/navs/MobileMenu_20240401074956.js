...
import { buyMeCoffeenURL } from '@lib/routes';
                ...
                <div className="relative flex flex-col items-start justify-start w-full h-full  px-4 pt-2 rounded-s-lg border border-transparent transition-colors border-neutral-600 bg-neutral-800 bg-opacity-90 z-40">
                    ...
                    <div className="flex flex-col flex-1 rounded-tl-lg gap-3 custom-scrollbar overflow-x-hidden overflow-y-scroll transform  w-full sm:w-full">
                        <div className='flex justify-between gap-4 mt-3'>
                            <h2 onClick={() => setIsNav(true)} className={`${isNav ? 'border border-zinc-700 rounded-lg' : ''} flex items-center justify-center text-xl font-bold w-1/2 py-3 text-gray-300 cursor-pointer`}>Navs</h2>
                            <h2 onClick={() => setIsNav(false)} className={`${!isNav ? 'border border-zinc-700 rounded-lg ' : ''} flex items-center justify-center text-xl font-bold w-1/2 py-3 text-gray-300 cursor-pointer`}>Categories</h2>
                        </div>
