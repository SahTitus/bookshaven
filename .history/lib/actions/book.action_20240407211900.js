...
export const fetchBook = async (slug) => {
    try {
        ...
        const populatedBook = await Book.findOne({ slug: 'the-last-dreams' })
...
export async function updateBook(bookData, user_id) {
    try {
        await connectDb();

        const creator = await User.findById(user_id,user_id);
        console.log(bookData,'mmmmmmmmmmmmmmmm')
                        // if (!creator.isAdmin) {
        //     throw new Error('Unauthorized access')
        // }

        const bookToUpdate = await Book.findById(bookData._id)
        ...
        const updatedBook = await Book.findByIdAndUpdate(bookData._id, bookData, { new: true });
