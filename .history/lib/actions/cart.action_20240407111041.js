...
export const fetchCartItems = async (userId) => {
    ...
    try {
        ...
        const cart = await Book.find({_id: userId})
            .sort({ publishedAt: 'desc' })

        const cartCount = await Book.countDocuments(conditions)

        return { data: JSON.parse(JSON.stringify(cart)), totalItems: cartCount }
    } catch (error) {
        handleError(error.message);
    }
};
