...
export const deleteCart = async ({cart_id, user_id}) => {
    try {
        ...
        const itemToDelete = await Cart.findById(cart_id)

        if (!itemToDelete._id || itemToDelete.userID != userData._id.toHexString()) {
            throw new Error('Unauthorized or Item not found')
        }
...
export const orderBooks = async (order) => {

    try {
        await connectDb();

        const user = await User.findById(Orde.userID);

        if (!user._id) {
            throw new Error("User does exist");
        }

        const newOrde = await Order.create(Orde);
        revalidatePath('/')

        return JSON.parse(JSON.stringify(newOrde));
    } catch (error) {
        handleError(error.message)
        return JSON.parse(JSON.stringify('Something went wrong'));
    }
