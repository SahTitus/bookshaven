...
export const PageContent = ({ book }) => {
    const { handleQuantityChange, quantity, setShowLoaderOverlay, setTotalCartQnty, setCartBooks } = useStateContex();
    ...
    const handleDelete = async () => {
        ...
        try {
            await deleteBook('66117947297f8027a53c8ded', user?.id)
            setShowLoaderOverlay(false)
            router.push('/')
        } catch (error) {
            setShowLoaderOverlay(false)
            handleError(`POST-UPDATE ARTICLE ERR:${error}`)
        }
    }
(
        ...
        <div>
            <div className='flex flex-col h-fit pb-6 sm:flex-row sm:items-start'>
                ...
                >
                    ...
                    />
                    {user?.isAdmin && <div className='flex items-center mt-4 mb-8 justify-between  bg-[#edbc37]  shadow-lg  z-20 rounded-full h-[45px] w-[13rem] text-white '>
                        <p onClick={handleDelete} className='text-3xl h-full bg-[#f8d985] cursor-pointer flex text-center justify-center items-center w-full rounded-l-full'><MdDelete /></p>
                        <span className='text-2xl font-semibold px-6'></span>
                        <Link
                            href={`/post?slug=${book?.slug}`}
                            onClick={() => handleQuantityChange(1)}
                            className='text-3xl h-full  cursor-pointer  bg-[#f0b513] rounded-r-full flex text-center justify-center items-center w-full'
                        >
                            <MdEdit />
                        </Link>
                ...
                <div className='flex flex-col mt-4 px-4'>
                    ...
                    <div className='flex justify-between items-center gap-2 mt-14 z-0' >
